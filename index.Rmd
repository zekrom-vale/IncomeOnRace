---
title: "Awesome Paper"
author:
- name: Zachary Bisson
  email: zachary.bisson@my.uwrf.edu
  affiliation: University of Wisconsin-River Falls
  footnote: 2
- name: Anna Elfstrum
  email: anna.elfstrum@my.uwrf.edu
  affiliation: University of Wisconsin-River Falls
  footnote: 2
- name: Shawn Graven
  email: shawn.graven@my.uwrf.edu
  affiliation: University of Wisconsin-River Falls
  footnote: 2
- name: Jack Kuivanen
  email: jack.kuivanen@my.uwrf.edu
  affiliation: University of Wisconsin-River Falls
  footnote: 2
- name: Caleb Olson
  email: caleb.olson@my.uwrf.edu
  affiliation: University of Wisconsin-River Falls
  footnote: 2
- name: Kate Soukkala
  email: katelyn.soukkala@my.uwrf.edu
  affiliation: University of Wisconsin-River Falls
  footnote: 2
date: "`r Sys.Date()`"
output:
  html_document: default
  pdf_document: default
footnote:
- code: 2
  text: Equal contribution, author order is alphabetical
abstract: |
  This is the abstract.
  
  It consists of two paragraphs.
journal: Econometrics (ECON 426)
address:
- code: University of Wisconsin-River Falls
  address: University of Wisconsin-River Falls, 410 South Third Street, River Falls,
    WI, 54022
bibliography: citations.bib
numbersections: yes
csl: core/elsevier-harvard.csl
preamble: |
  \usepackage{natbib}
editor_options:
  markdown:
    wrap: 100
---

```{r, eval=FALSE, echo=FALSE, include=FALSE}
file.edit("markown/_head.Rmd")
```


```{r, child="markown/_head.Rmd"}
```

# Introduction
```{r, child="markown/Introduction.Rmd"}
```


# Literature Review

@Keating2019

Here are two sample references: [@pais_socioeconomic_2011; @bob].

# Theoretical Analysis

$$y=\beta_0+\beta_1 x$$

# Empirical Analysis

## Data
```{r, child='markown/importAndClean.Rmd'}
```

```{r, child='markown/Export.Rmd'}
```

## Methodology

## Results

```{r, child='markown/Estimators.Rmd'}
```

```{r, child='markown/WageoverTime.Rmd'}
```

```{r, child='markown/WageAccordingtoJob.Rmd'}
```


# Conclusion


# References
<!-- All of you references should be stored in BibTeX format in the file ../bib_file.bib -->
